...
var deploy = function deploy(app, branch) {
  ...
  _logger.default.queue(_logger.default.pretty(_app.name, 'running', "deploy ".concat(branch)), _app.name, 'info', true); // fetch repo
  ...
  return _git.default.fetch(_app.name, dir).then(function () {
    ...
    return (0, _deployer.runDeployScript)(_app.script, branch, _app.name, _config.default.projectDir);
  }).then(function () {
    // replay logs
